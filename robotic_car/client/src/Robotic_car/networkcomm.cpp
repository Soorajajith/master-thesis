...
struct sockaddr_in udp_si_other, server_prediction;
int udp_socket, slen, socket_prediction, slen_prediction;
WSADATA wsa, wsaData;
...
void closeNetworkPrediction() {
	closesocket(socket_prediction);
	WSACleanup();
}
...
int connectPrediction(char* addr) {
	slen_prediction = sizeof(struct sockaddr_in);
	...
	socket_prediction = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
	if (socket_prediction == SOCKET_ERROR)
	{
		printf("socket() failed with error code : %d\n", WSAGetLastError());
		closeNetworkPrediction();
		return -1;
	...
	memset((char *)&server_prediction, 0, sizeof(server_prediction));
	server_prediction.sin_family = AF_INET;
	server_prediction.sin_port = htons(8889);
	server_prediction.sin_addr.S_un.S_addr = inet_addr(addr);
...
int getPrediction(char* buf) {
	int recv = recvfrom(socket_prediction, buf, BUFLEN, 0, (struct sockaddr*)&server_prediction, &slen_prediction);
	return recv;
}
...
int sendImageToNetwork(IplImage* buf) {
	int send = sendto(socket_prediction, buf->imageData, BUFLEN_IMG, 0, (struct sockaddr *)&server_prediction, slen_prediction);
...
std::string printBinary(char* buf, int len){
	std::stringstream ss;
	char byte;
	int bit;
	for (int i = 0; i < len; i++) {
...
void instructionByte(char* byte, int len){
	instruction = printBinary(byte, len);
}
...
void sendMovementData(char* move){
	...
	if (move[1] != old_move[1] || move[2] != old_move[2]){
		...
		old_move[1] = move[1];
		old_move[2] = move[2];
		instructionByte(move, 3);
	}
}

std::string getInstructionByte(){
	return instruction;
}
